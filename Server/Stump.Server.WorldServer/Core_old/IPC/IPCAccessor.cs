...
namespace Stump.Server.WorldServer.Core.IPC
{
    public class IPCAccessor : IPCEntity
    {
        ...
        public void Start()
        {
            ...
            Running = true;
            TaskPool.Start();
            Tick();// the first tick dont take in timer
            m_updateTimer.Start();
        ...
        {
            var clients = WorldServer.Instance.FindClients(client => client.Account != null && client.Account.Id == message.AccountId).ToList().ToArray();

    ...
    public class WorldClientCollection : IPacketReceiver, IDisposable
    {
        ...
        public void Remove(WorldClient client)
        {
            lock (m_lock)
            {
                ...
                foreach (var client_temp in m_underlyingList.ToList())// to remove the character exactly in multi accounts, but idk this just work when i pause in debbuger ;/
                {
                    if (client_temp.Character != null && client.Character != null)
                    {
                        if (client_temp.Character == client.Character)
                        {
                            m_underlyingList.Remove(client);
                            break;
                        }
